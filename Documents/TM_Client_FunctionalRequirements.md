...
- **FR1.1** Present the **6 config areas** and let the Admin move between them via tabs: Capture Rules, Privacy, Data Connection, Client Behaviour, User Attributes, Integrations. **Privacy** and **Integrations** are container areas whose sub-screens are presented as **sections**— Privacy: Redaction · Hashing · Consent; Integrations: SAP · Extensions.
...
- **FR15.2** **Rollout state is always visible**, beside the page title (not among the actions — it describes the configuration, not an action). The indicator answers **one** question: *does what I am looking at match what the machines are running?*
  ...
  - **The note appears only in the Draft state**, where it names the outstanding action — *"changes not yet applied"*. Live needs no note: repeating the date a few pixels from the subtitle that already carries it adds nothing.
  - **The page subtitle carries the rollout fact**, and it is **the same on every tab** — *"Last applied on 4 Aug 2026, 19:45"*, or *"Nothing has been applied yet — no activity is being captured."* before a first apply. It must **not** be area-specific: the subtitle sits under a page-level title, so text that changes as the Admin moves between tabs reads as though the page changed when only the tab did. The active tab already names the area. *(Changed 2026-08-04: the subtitle previously held a different sentence per area.)*
  - **The pill, not the note, distinguishes whether anything is live.** **Not applied** means nothing has ever gone out; **Draft** means an older configuration is running. The note is not required to carry that.
  - **Wording differs between the two states deliberately.** *"Draft · last applied on 4 Aug"* reads as though the **draft** had been applied — the nearest noun becomes the subject of "applied". Naming the action instead avoids that reading entirely.
  - **A disabled Apply must state its reason accessibly.** The reason (*"Save your draft first"*, or a validation message) is carried in the button's **accessible label**, not in a hover title alone, which reaches neither keyboard nor touch users. The enabled **Save draft** button beside it provides the visual cue.
  - *(Changed after design review, 2026-08-04. v3 originally specified three single-pill states that compared only the **saved draft** against the **applied** configuration and never looked at the screen, so the pill kept reading "Live for all users" while the Admin was mid-edit. Caught by Diego Jaramillo, reproduced in the prototype, and resolved after building and comparing three candidate models. See decision #51.)*
...
- **FR15.4** **Apply asks for confirmation.** A dialog states plainly that the settings **take effect for every user in this project straight away, replacing whatever is running now**, with **Cancel** and a primary **Apply**; Escape cancels. **No inputs are captured** — no approver name, ticket reference or evidence.
  - **The copy must stay generic — it must not name review or approval.** An earlier version added *"only continue if this configuration has been reviewed and approved"*. That was removed after PM review (2026-08-04): a review-and-approve step exists at **one** customer, so wording it into the dialog imposes one organisation's compliance process on every other project. The dialog's job is to state the **effect** of applying; whether an approval preceded it is the customer's process, not the product's claim.
  ...
  - **The action is called "Apply", not "Apply to all users".** Naming the audience invites *"can we apply to only some users?"*, which is explicitly not a direction the product wants to go. The dialog body still states that everyone is affected, because that is the consequence the Admin needs to weigh. *(Changed after PM review, 2026-08-04.)*
...
- **FR15.11** **Upload configuration file** — the counterpart to FR15.5, also in **More actions**. After sending a file for approval the Admin may carry on editing, so the saved draft can drift from what was actually signed off. Uploading the approved file **restores it exactly**, making the approved artefact and the applied artefact the same object again. *(This is why drift detection is unnecessary: rather than warning that the two differ, the Admin can make them identical.)*
  ...
  - **Validate and reject outright — never partially apply.** Unreadable JSON, a file that isn't a configuration, missing sections, no capture rules, or no Default rule are each reported by name, and the current configuration is left untouched.
  - Errors use a **single-action notice** (OK only) — there is nothing to confirm or cancel, and offering Cancel would imply the change might still happen.
